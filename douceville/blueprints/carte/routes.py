...
@carte.route("/points", methods=["GET"])
def get_all_points():
...
@carte.route("/isochrone", methods=["GET"])
def isochrone():
    token = request.args.get("token", "")
...
def show():
    ...
    return render_template(
        "carte/carte.html",
        points_request="%s:%i/carte/points?token=%s" % (Config.HOST, Config.PORT, token),
        isochrone_request="%s:%i/carte/isochrone?token=%s"
